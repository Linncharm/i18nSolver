...
export class I18nParser {
        ...
        traverse(ast, {
            ...
            ObjectProperty: (path) => {
                if (
                    ...
                    if (callExprPath && t.isCallExpression(callExprPath.node) && t.isIdentifier(callExprPath.node.callee)) {
    ...
    public parse(sourceCode: string): { code: string, translations: Record<string, string> } {
        ...
        traverse(ast, {
            StringLiteral: (path) => {
                ...
                if (t.isJSXAttribute(path.parent)) {
                    ...
                    if (jsxElement) {
                        const componentName = ((jsxElement.node as t.JSXElement).openingElement.name as t.JSXIdentifier).name;
                ...
                if (t.isObjectProperty(path.parent) && t.isIdentifier(path.parent.key)) {
                    ...
                    if (callExprPath && t.isCallExpression(callExprPath.node) && t.isIdentifier(callExprPath.node.callee)) {
                        ...
                        // Replace with t() call
                        path.replaceWith(
                            t.callExpression(t.identifier('t'), [t.stringLiteral(keyName)])
                        );
                    }
                }
            },
    ...
    public parseFile(filePath: string, outputPath?: string): { code: string, translations: Record<string, string> } {
